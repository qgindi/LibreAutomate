...
if (args[0] == "post") {
	...
	run.console(out var so, "robocopy.exe", $"\"{source}\" \"{outputPath}\" /e /sj /sl /r:2 /w:1 /xd Git SDK .compiled .temp .state /xf *.pdb Au.dll Au.Controls.dll Au.Editor.dll *.deps.json *.runtimeconfig.json Microsoft.Web.WebView2.* NuGet.*");
