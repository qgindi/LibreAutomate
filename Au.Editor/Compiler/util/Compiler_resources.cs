	...
	unsafe class _NativeResources {
		...
		public void WriteAll(string exeFile, byte[] exeData, MCPlatform platform, bool console) {
			fixed (byte* pBase = exeData) {
				...
				//The apphost template does not contain resources, therefore .rsrc section does not exist and ImageDirectoryEntryToDataEx fails.
				...
				if (platform == MCPlatform.x86) {
					...
					oh32->Subsystem = (ushort)(console ? 3 : 2);
					oh32->CheckSum = 0;
				} else {
					...
					oh->Subsystem = (ushort)(console ? 3 : 2); //IMAGE_SUBSYSTEM_WINDOWS_CUI/GUI
					oh->CheckSum = 0;
				}
