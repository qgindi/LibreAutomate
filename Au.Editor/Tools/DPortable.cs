...
class DPortable : KDialogWindow {
	...
	public DPortable() {
		...
		if (filesystem.comparePaths(_dirThisApp, _dirNet) is CPResult.Same or CPResult.AContainsB) _dirNet = _dirNetDesktop = null;
		...
		tDir.TextChanged += (_, _) => {
			...
			if (_exists) {
				...
				if (filesystem.getFinalPath(_dirPortableApp, out var s1, format: FPFormat.PrefixAlways) && filesystem.getFinalPath(data, out var s2, format: FPFormat.PrefixAlways)) {
	...
	void _InstallThread() {
		...
		void _CopyData() {
			...
			if (_dirData.Eqi("data")) {
				...
				Directory.CreateSymbolicLink(data, _dirData); //probably relative link
		...
		void _WorkspaceAndSettings() {
			...
			switch (filesystem.comparePaths(ref doc1, ref ws1)) {
		...
		void _DeleteOldData() {
			...
			if (filesystem.exists(data)) {
				...
				if (filesystem.getFinalPath(data, out var s)) filesystem.delete(s); //if symlink, delete target
	...
	void _PrintDetails() {
		...
		bool wsInDoc = filesystem.comparePaths(_dirThisAppDocuments, _dirWorkspace) == CPResult.AContainsB;
...
""");
		...
		void _Links(StringBuilder b) {
			...
			foreach (var f in App.Model.Root.Descendants()) {
				if (f.IsLink || f.IsSymlink) {
					...
					if (f.IsSymlink) { filesystem.getFinalPath(s, out s); s2 = "Folder "; }
	...
	string _ValidateFolder() {
		...
		if (filesystem.exists(_dirPortableApp)) {
			bool bad = filesystem.comparePaths(_dirPortableApp, _dirWorkspace) is CPResult.AContainsB or CPResult.BContainsA or CPResult.Same;
			if (!bad) {
				foreach (var f in App.Model.Root.Descendants()) {
					if (f.IsLink || f.IsSymlink) {
						var s = f.FilePath;
						if (filesystem.comparePaths(_dirPortableApp, s) is CPResult.AContainsB or CPResult.BContainsA or CPResult.Same) {
							if (!bad) print.it("Links to portable:");
							bad = true;
							if (f.IsSymlink) filesystem.getFinalPath(s, out s);
