...
static class TUtil {
	...
	/// Returns null if <b>Editor.Settings.edit_noImages</b>.
	/// </summary>
	...
	public static string MakeScreenshot(POINT p, CapturingWithHotkey capt = null) {
		if (Editor.Settings.edit_noImages) return null;
	...
	/// <summary>
	/// Adds link +hotkey that shows dialog "Hotkeys" and updates Editor.Settings.delm.hk_x.
	/// </summary>
	public static void RegisterLink_DialogHotkey(KSciInfoBox sci) {
		...
		sci.AaTags.AddLinkTag("+hotkey", _ => {
			...
			b.R.Add("Capture", out TextBox capture, Editor.Settings.delm.hk_capture).xValidateHotkey(errorIfEmpty: true).Focus();
			b.R.xAddGroupSeparator("In elm tool");
			b.R.Add("Insert code", out TextBox insert, Editor.Settings.delm.hk_insert).xValidateHotkey();
			b.R.Add("Capturing method", out TextBox smaller, Editor.Settings.delm.hk_smaller).xValidateHotkey();
			...
			if (b.ShowDialog(Window.GetWindow(sci))) {
				Editor.Settings.delm.hk_capture = capture.Text;
				Editor.Settings.delm.hk_insert = insert.TextOrNull();
				Editor.Settings.delm.hk_smaller = smaller.TextOrNull();
			}
		});
	}
