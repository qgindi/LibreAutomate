...
void LineMarkers::RemoveLine(Sci::Line line) {
	...
	if (markers.Length()) {
#if !true //Au: remove this feature. Rejected, because also deletes marker when shouldn't, eg when joining the next line with the marker line. Never mind.
		markers.Delete(line - 1);
		Platform::DebugPrintf("%i", (int)line); //sometimes line is 1-based, sometimes 0-based. I don't get it.
#else
		if (line > 0) {
			MergeMarkers(line - 1);
		}
