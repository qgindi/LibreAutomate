...
public static unsafe class osVersion {
	...
	static readonly Lazy<string> _environment = new(() => $"{_winmajor.ToS()}.{_winminor.ToS()}.{_winbuild.ToS()}-{(_is32BitOS ? "32" : "64")}|{Environment.Version}|{typeof(osVersion).Assembly.GetName().Version}");
}
