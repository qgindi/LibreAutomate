/// It is recommended to keep toolbars and keyboard/autotext/mouse/window triggers in folder <q>@Triggers and toolbars<>. It contains script <q>Triggers and toolbars<>, split into multiple files. Click a <b>TT<> menu item to open a file; it creates the folder/file if need.
/// 
...
/// 
/// How it works: When the script starts, it calls functions with attribute <q>[Triggers]<> or <q>[Toolbars]<>. They are in other files; they contain triggers and toolbars (you added them there). Then the script runs all the time, waiting for trigger/toolbar events (hotkey, button click, etc). On event it executes the trigger/button action (code that follows the <q>=><>).
/// 
...
/// 
/// To disable a trigger, comment out the trigger code line. Then click <b>Run<> to restart. To disable/enable all triggers, use the tray icon menu or menu <b>TT<>. Also you can comment out the <q>[Triggers]<> attribute to disable all triggers defined there. See also class <help Au.Triggers.ActionTriggers>ActionTriggers<> examples.
/// 
/// Avoid multiple scripts with triggers running all the time. Each script uses some CPU, which makes the computer slightly slower. Particularly input events (key, mouse) and window events. That is why triggers should be in folder <q>@Triggers and toolbars<>.
/// 
